...
arma::rowvec BS2(double Time, arma::vec V, arma::vec S, double FirstInf,
  ...
  if(V(0) > 0){
    for(int j = 0; j < n_v; ++j){
      ...
      } else if(V(j) >= SV_ind & V(j) < infection_ind){
        if(Time > S(j) & S(j) > FirstInf){ B.subvec(start, end) = BS(Time - S(j), knots_j, constantVE);}
      } else{
...
struct CoxPHInformation get_Info(arma::vec beta, arma::vec gamma, arma::vec d,
                                 ...
                                 arma::field<arma::vec> knots, bool constantVE, arma::vec dimension, arma::mat Z,
                                 arma::vec Score_constant, int infection_ind, int SV_ind, arma::vec VacTime, arma::vec FirstInfTime,
                                 double cutoff){
...
#pragma omp parallel for
  for(int k = 0; k < K; ++k){
    ...
    for(int j = 0; j < index.n_elem; ++j){
      ...
      arma::rowvec Zk = BS2(tk, V_index_j, S_index_j, FirstInf_index_j, knots, dimension, constantVE, infection_ind, SV_ind);
...
Rcpp::List Cox_general(arma::vec Time, arma::vec t, arma::vec Delta, arma::vec C, arma::field<arma::vec> V,
                       arma::field<arma::vec> S, arma::mat X, arma::vec m, arma::mat Xg, arma::vec ng,
                       double eps, int MaxIter, arma::field<arma::vec> knots, bool constantVE,
                       arma::vec dimension, int infection_ind, int SV_ind, arma::vec VacTime, arma::vec FirstInfTime,
                       double cutoff){
  Rcpp::List ret;
  int tau = max(Time);
...
#pragma omp parallel for
  for(int i = 0; i < n; ++i){
    ...
    for(int j = 0; j < m(i); ++j){
      Z.row(start_i+j) = BS2(Time_i(j), V_i, S_i, FirstInf_i, knots, dimension, constantVE, infection_ind, SV_ind);
    }
  }
  std::cout << "Z completed" << std::endl;
  ...
  std::cout << "Score_constant completed" << std::endl;

  temp_old = get_Info(theta_old.head(p1), theta_old.tail(p2), d, Time, t, Delta, C, V, S, X, m,
                      Xg, ng, knots, constantVE, dimension, Z, Score_constant, infection_ind, SV_ind, VacTime, FirstInfTime, cutoff);
  std::cout << "Initial Info completed" << std::endl;
  while(error > eps && Iter < MaxIter){
    theta = theta_old + arma::solve(temp_old.InformationMat, temp_old.ScoreFun);
    std::cout << "beta = " << theta.head(p1).t() << std::endl;
    std::cout << "gamma = " << theta.tail(p2).t() << std::endl;
    temp_new = get_Info(theta.head(p1), theta.tail(p2), d, Time, t, Delta, C, V, S, X, m,
                        Xg, ng, knots, constantVE, dimension, Z, Score_constant, infection_ind, SV_ind, VacTime, FirstInfTime, cutoff);
  ...
  beta = theta.head(p1);
  gamma = theta.tail(p2);
  arma::mat cov = arma::inv(temp_new.InformationMat);
