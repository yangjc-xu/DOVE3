...
arma::rowvec BS2(double Time, arma::vec V, arma::vec S, arma::field<arma::vec> knots, arma::vec dimension, bool constantVE, int infection_ind){
...
struct CoxPHInformation get_Info(arma::vec beta, arma::vec gamma, arma::vec d,
                                 ...
                                 arma::vec Score_constant, int infection_ind, arma::vec VacTime, double cutoff){
  ...
  for(int k = 0; k < K; ++k){
    ...
    for(int j = 0; j < index.n_elem; ++j){
      ...
      arma::rowvec Zk = BS2(tk, V_index_j, S_index_j, knots, dimension, constantVE, infection_ind);
...
// [[Rcpp::export]]
Rcpp::List Cox_booster(arma::vec Time, arma::vec t, arma::vec Delta, arma::vec C, arma::field<arma::vec> V,
                       arma::field<arma::vec> S, arma::mat X, arma::vec m, arma::mat Xg, arma::vec ng,
                       double eps, int MaxIter, arma::field<arma::vec> knots, bool constantVE,
                       arma::vec dimension, int infection_ind, arma::vec VacTime, double cutoff){
  Rcpp::List ret;
  int tau = max(Time);
  ...
  for(int i = 0; i < n; ++i){
    ...
    arma::vec S_i = S(i);
    for(int j = 0; j < m(i); ++j){
      Z.row(start_i+j) = BS2(Time_i(j), V_i, S_i, knots, dimension, constantVE, infection_ind);
    }
  }
  ...
  temp_old = get_Info(theta_old.head(p1), theta_old.tail(p2), d, Time, t, Delta, C, V, S, X, m,
                      Xg, ng, knots, constantVE, dimension, Z, Score_constant, infection_ind, VacTime, cutoff);
